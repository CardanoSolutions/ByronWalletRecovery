...
module Pos.Binary.Class.Core where
...
defaultEncodeList xs = E.encodeListLenIndef <> foldr (\x r -> encode x <> r) E.encodeBreak xs
...
instance Bi Char where
    ...
    encodeList cs = E.encodeString (T.pack cs)
    decodeList    = do txt <- D.decodeString
                       return (T.unpack txt) -- unpack lazily
...
instance Bi a => Bi (NonEmpty a) where
  encode = defaultEncodeList . NE.toList
  decode = do
    l <- defaultDecodeList
    case nonEmpty l of
...
instance (Hashable k, Ord k, Bi k, Bi v) => Bi (HM.HashMap k v) where
  encode = encodeMapSkel HM.size $ \f acc ->
      ...
      foldr (uncurry f) acc . sortOn fst . HM.toList
